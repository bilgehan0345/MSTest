...
#define NEXTION_RX_PIN                                                         \
  16 // ESP32 RX2 (Nextion'un TX - Mavi kablosu buraya takılacak)
#define NEXTION_TX_PIN                                                         \
  17 // ESP32 TX2 (Nextion'un RX - Sarı kablosu buraya takılacak)
...
// ---------------------------------------------------------------------------
#define GEAR_RATIO                                                             \
  1.0 // Vites / Redüktör Oranı (Doğrudan tekerleğe bağlıysa 1.0)
#define WHEEL_RADIUS_M                                                         \
  0.30 // Tekerlek Yarıçapı (metre cinsinden, örn: 30cm = 0.30)
...
void sendNextionCommand(String cmd) {
  ...
  Serial2.write(0xFF);
  Serial2.write(0xFF);
  // Hata ayıklama (Debug) için seri porta da yazdırıyoruz
  Serial.println("[NEXTION GIDEN] " + cmd);
}
...
void setup() {
  ...
  if (mcp2515.setBitrate(CAN_BAUD_RATE, MCP2515_OSCILLATOR_FREQ) !=
      MCP2515::ERROR_OK) {
...
void loop() {
  ...
  if (mcp2515.readMessage(&rxMsg) == MCP2515::ERROR_OK) {
    ...
    if (rxMsg.can_id == 0x123) {
      // Gelen 8 byte'ın ilk iki byte'ını (data[0] ve data[1]) RPM olarak
      // birleştiriyoruz.
  ...
  // 2. ADIM: Gerçekçilik katmak için motoru yavaş yavaş hedef RPM'e yaklaştır
  // (Atalet Simülasyonu) Her 50ms'de bir devri hedefe doğru ufak ufak
  // artır/azalt. İsterseniz direkt "rpm = targetRpm;" de yapabilirsiniz.
  if (millis() - lastRpmUpdateTime > 50) {
    ...
    if (abs(rpm - targetRpm) <= 20)
      rpm = targetRpm;
  }

  // 3. ADIM: Her 500ms'de bir kendi anlık (gerçek) devrimizi karşı tarafa
  // bildir.
  if (millis() - lastSendTime > 500) {
    ...
    if (err == MCP2515::ERROR_OK) {
      ...
      if (rpm != lastPrintedRpm) {
        // Hız (km/h) = (RPM / Dişli Oranı) * Tekerlek Çevresi(m) * 60(dk) /
        // 1000(m)
        int speed = (rpm / GEAR_RATIO) * WHEEL_CIRCUMFERENCE * 60.0 / 1000.0;

        Serial.printf("CAN TX -> ID: 0x%X | Hedef RPM: %d | Gercek RPM: %d | "
                      "Hiz: %d km/h\n",
                      txMsg.can_id, targetRpm, rpm, speed);

        // Nextion ekrana verileri gönder
        // Kullanıcı objname'i veya tipini (Number vs Text) yanlış seçmiş olabilir diye brute-force yapıyoruz:
        sendNextionCommand("rpm.val=" + String(rpm));
        sendNextionCommand("rpm.txt=\"" + String(rpm) + "\"");
        sendNextionCommand("n0.val=" + String(rpm));
        
        sendNextionCommand("speed.val=" + String(speed));
        sendNextionCommand("speed.txt=\"" + String(speed) + "\"");
